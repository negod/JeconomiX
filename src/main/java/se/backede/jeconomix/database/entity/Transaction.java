...
@NamedQueries({
    ...
    @NamedQuery(name = EntityQueries.TRANSACTION_BY_BUDGETYEAR_AND_CATEGORY, query = "select t from Transaction t where t.budgetYear =:budgetYear AND t.company.category.categoryType.type =:categoryType"),
    @NamedQuery(name = EntityQueries.TRANSACTION_BY_BUDGETYEAR, query = "select t from Transaction t where t.budgetYear =:budgetYear")
})
